...
class Instructions(Page):
    ...
    def multiple1_choices(self):
        return [[1, self.player.senior_name], [-1, self.player.junior_name],
                [0, "I'm equally likely to get each of the two."]]
    ...
    def error_message(self, values):
        ...
        if values["truefalse4"] != True:
            summand += 1
        if self.player.high_acc != self.player.low_acc and values["multiple1"] != (self.player.senior_prob >= 0.5) - (
            self.player.senior_prob <= 0.5):
            summand += 1
        if values["blank1"] != Constants.treatment_dict['good_prior'][self.player.treatment]:
            summand += 1
        if self.player.high_acc != self.player.low_acc and values["blank2"] != Constants.treatment_dict['high_acc'][
...
class Task(Page):
    ...
    def is_displayed(self):
        return self.player.participant.vars.get(
            'failure') < Constants.failuretolerance  # COMMENT THIS OUT WHEN RUNNING THE EXPERIMENT!!!
...
class Survey(Page):
    ...
    def is_displayed(self):
        return self.player.participant.vars.get(
            'failure') < Constants.failuretolerance and self.round_number == Constants.num_rounds
